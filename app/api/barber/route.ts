...
export async function GET(request: Request) {
  ...
  const [appointments, overrides, services, blocks] = await Promise.all([
    ...
    db.from("availability_blocks").select("id, start_date, end_date, label").eq("barber_id", barber.id).gte("end_date", new Date().toISOString().slice(0, 10)).order("start_date"),
  ]);
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    if (action === "block_range") { const startDate = String(body.startDate ?? ""), endDate = String(body.endDate ?? ""), label = String(body.label ?? "Cierre").slice(0, 80); if (!datePattern.test(startDate) || !datePattern.test(endDate) || endDate < startDate) return Response.json({ error: "El rango de fechas no es válido." }, { status: 400 }); const result = await db.from("availability_blocks").insert({ barber_id: barber.id, start_date: startDate, end_date: endDate, label }); if (result.error) throw result.error; return Response.json({ ok: true }); }
